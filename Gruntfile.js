module.exports = function(grunt) {
  ...
  grunt.initConfig({
    // javascript linting
    jshint: {
      files: ['Gruntfile.js', 'CommentVotes.js'],
      options: {
    ...
    exec: {
      ...
      deploy: {
        cmd: function(environment) {
          ...
          if(environment == 'development') {
            checkout = 'master';
            functionName = 'devCommentVotes';
          } else if(environment == 'production') {
            checkout = 'production';
            functionName = 'CommentVotes';
          } else {
            return '';
          }
          var deployCommand = 'aws lambda update-function-code --function-name ' + functionName + ' --zip-file fileb://CommentVotes.zip --region eu-west-1 --profile weco';
          return 'echo Checking out ' + checkout + ' && git checkout ' + checkout + ' && echo Deploying... && ' + deployCommand + ' && git checkout master';
        }
      }
    },
    zip: {
      'CommentVotes.zip': ['CommentVotes.js', 'node_modules/**/*']
    }
  });
